...
namespace MovieLibrary
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            // while => while (E) S;
            // 0+ oterations, pre test condition
            while (true)
            {
        ...
        static int ReadInt32 ( int minimumValue )
        {
            do
            {
                ...
                //  Input/output parameter ("pass by reference") - a reference to the argument is passed to the function and both original argument and aparameter reference same value ( C++: int& )
                //  Output parameter - function caller does not pprovide input, function always provides the output (C++: return type)
